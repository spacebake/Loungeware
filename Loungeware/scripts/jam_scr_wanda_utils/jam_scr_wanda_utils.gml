...
function jam_draw_witch(_witch_spr, _witch_img, _broom_spr, _broom_img, _x, _y, _xscale, _yscale, _colour, _alpha, _dx=0, _dy=0) {
...
function jam_map_range(_value, _min1, _max1, _min2, _max2) {
	var dx = _max1 - _min1;
	var dy = _max2 - _min2;
...
function jam_movement_iterate(_prev_x, _acc, _frict, _handling, _dir) {
    var new_x;
    if (_dir == 0) {
...
function jam_wanda_spawn_particles(_sys) {
    static ty = (function() {
...
/// @param {real} sys The ID of the particle system to use.
/// @param {real} sys The ID of the particle system to use.
function jam_wanda_enemy_spawn_particles(_sys) {
    static ty = (function() {
        var _ = part_type_create();
...
function jam_wanda_sealine_spawn_particles(_sys, _left, _top, _right, _bottom) {
    static ty = (function() {
        var _ = part_type_create();
        part_type_sprite(_, jam_spr_wave, false, false, true);
...
/// @param {real} x2 The right position of the region to spawn particles in.
/// @param {real} y2 The bottom position of the region to spawn particles in.
function jam_wanda_skyline_spawn_particles(_sys, _left, _top, _right, _bottom) {
    static ty = (function() {
        var _ = part_type_create();
        part_type_sprite(_, jam_spr_cloud, false, false, true);
...
/// @param {real} x2 The right position of the region to spawn particles in.
/// @param {real} y2 The bottom position of the region to spawn particles in.
function jam_wanda_island_spawn_particles(_sys, _left, _top, _right, _bottom) {
    static ty = (function() {
        var _ = part_type_create();
        var life = 60 * 20;
        part_type_sprite(_, jam_spr_island_floating, false, false, true);
...
function jam_draw_text_3d(_x, _y, _text, _layer_count, _outline, _fill) {
    static sep = 1;
    ...
    for (var i = 0; i < n; i += 1) {
        ...
        draw_set_colour(col);
        jam_draw_text_wonky(_x + i * dx, _y + i * dy, _text);
    }
}
...
/// @param {real} y The y position to draw the text at.
/// @param {real} text The text to draw.
function jam_draw_text_wonky(_x, _y, _text) {
...
function jam_easein(_interp) {
    static chan = animcurve_get_channel(jam_ac_ease, 0);
    return animcurve_channel_evaluate(chan, _interp);
}

/// @desc Applies mid-slow interpolation.
/// @param {real} amount The amount to interpolate.
function jam_midslow(_interp) {
    static chan = animcurve_get_channel(jam_ac_ease, 1);
    return animcurve_channel_evaluate(chan, _interp);
}
...
/// @author Kat @katsaii
function jam_lerp_bezier(_amount, _x1, _y1, _x2, _y2, _x3, _y3) {
...
function jam_spawn_enemy(_x, _y, _amp_x, _amp_y) {
    with (instance_create_layer(_x, _y, layer, jam_obj_enemy)) {
        amplitudeX = _amp_x;
        amplitudeY = _amp_y;
    }
}
