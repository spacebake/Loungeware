...
void main()
{
    ...
    alpha = max(alpha, texture2D(gm_BaseTexture, v_vTexcoord + 1.0 * offset_x).a);
    alpha = max(alpha, texture2D(gm_BaseTexture, v_vTexcoord + 2.0 * offset_x).a);
    alpha = max(alpha, texture2D(gm_BaseTexture, v_vTexcoord - 1.0 * offset_x).a);
    alpha = max(alpha, texture2D(gm_BaseTexture, v_vTexcoord - 2.0 * offset_x).a);
    alpha = max(alpha, texture2D(gm_BaseTexture, v_vTexcoord + 1.0 * offset_y).a);
    alpha = max(alpha, texture2D(gm_BaseTexture, v_vTexcoord + 2.0 * offset_y).a);
    alpha = max(alpha, texture2D(gm_BaseTexture, v_vTexcoord - 1.0 * offset_y).a);
    alpha = max(alpha, texture2D(gm_BaseTexture, v_vTexcoord - 2.0 * offset_y).a);
