...
if (_has_result){
	...
	_has_score_data = variable_struct_exists(_result_data, "score_data");
}

/// @description 
if (async_load[? "id"] == request_id && _has_score_data){
	...
	if (_is_error){
		...
		leaderboard_data = variable_struct_get(_return_obj, "score_data");
		leaderboard_data = fill_blanks(leaderboard_data, max_score_count);
		data_retrieved = true;
	}
