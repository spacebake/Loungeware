...
else if wae_hog_state == "running"
{
	...
	if KEY_UP or KEY_PRIMARY
	{
		wae_hog_speed += 0.05
		wae_hog_myrider.image_index = 2
	}
	if (KEY_DOWN or KEY_SECONDARY) 
	{
		wae_hog_myrider.image_index = 3
		if  wae_hog_speed >= wae_hog_minspeed
		{
			wae_hog_speed -= 0.1
		}
	}
...
if DIFFICULTY > 2
{
	wae_hog_eff_speed = (wae_hog_speed + max(0, 0.5*(DIFFICULTY-2)- 0.3*abs(wae_hog_angspeed)))
}
else
{
	wae_hog_eff_speed = wae_hog_speed
}
if DIFFICULTY > 3
{
...
x += dcos(wae_hog_dir)*wae_hog_eff_speed
y -= dsin(wae_hog_dir)*wae_hog_eff_speed
...
image_speed =wae_hog_eff_speed*1 + abs(wae_hog_angspeed)/5
wae_hog_prevx = x
wae_hog_prevy = y