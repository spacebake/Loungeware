...
var xpos = floor(lerp(KATSAII_WITCH_WANDA_VIEW_RIGHT + 200, KATSAII_WITCH_WANDA_VIEW_CENTRE_X + dx, jam_easein(fadeIn)));
var ypos = floor(lerp(KATSAII_WITCH_WANDA_VIEW_CENTRE_Y + dy, KATSAII_WITCH_WANDA_VIEW_BOTTOM + 200, jam_easein(fadeOut)));
var tilt = 0;
if (global.jamHp < 10) {
...
draw_sprite_ext(jam_spr_main_island, 0, xpos, ypos, 1, 1, tilt, c_white, 1);
draw_set_alpha(0.9);
...
jam_draw_text_wonky(xpos, ypos + 30, "HEALTH");
...
draw_set_colour(c_white);
gpu_set_blendmode(bm_add);
var width = sprite_get_width(jam_spr_sheen);
...
for (var i = left; i < right; i += width) {
    ...
    draw_sprite(jam_spr_sheen, 0, i + offset, band_top);
}
gpu_set_blendmode(bm_normal);